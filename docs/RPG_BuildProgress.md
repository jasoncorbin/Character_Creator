...
> ## ▶ RESUME HERE (next session) — last updated 2026-06-18 (Phase C started: NoWeapon combat done)
> **PHASE C (per-stance combat) IN PROGRESS (2026-06-18):** Stance-aware combat montage selection is BUILT + PIE-CONFIRMED. Attack (light/heavy combo) + dodge (4-dir) `Play Montage` nodes now read per-stance `AnimMontage` arrays by `CurrentStance` (`StanceComboLight`, `StanceComboHeavy`, `StanceDodgeFWD/BWD/LFT/RGT`, size 8). NoWeapon(0) + SingleSword(1) filled & working. **Adding a stance is now DATA-ONLY — no graph edits:** (me) dup RM rolls→Edited + 4 dodge montages via `AnimMontageFactory` + populate StanceDodge[N]; (user) author 2 combo montages (duplicate SingleSword's + swap clips) + (me) populate StanceCombo[N].
> **▶▶ DO FIRST NEXT SESSION — finish TwoHandsSword(2) combat:** THS dodge is DONE (4 montages `AM_RPG_Dodge_{DIR}_TwoHandsSword` created from `Roll{DIR}_RM_THS` RM-fixed clips; `StanceDodge*[2]` populated). **NOT DONE: the 2 THS combo montages.** USER to author `AM_RPG_Combo_TwoHandsSword` (dup `AM_RPG_Combo_SingleSword` → swap clips to `Combo01/02/03_InPlace_THS`, UpperBody slot) + `AM_RPG_ComboHeavy_TwoHandsSword` (dup heavy → `Combo04/05_InPlace_THS`, DefaultSlot), into `/Game/RPG/Montages/TwoHandsSword/`. THEN (me) populate `StanceComboLight[2]`/`StanceComboHeavy[2]`, THEN PIE-test (Q→TwoHandsSword: 4-dir dodge + LMB combo). (Optional: if THS light combo looks stiff on UpperBody, move it to DefaultSlot.)
> Still open after THS: replicate combat to S&S(3)→Dual(4)→Spear(5)→Wand(6)→Bow(7); get-hit/death montages per stance (deferred, batchable via Python); Bow skeletal visual; MagicWand cast design. Phase C work UNCOMMITTED on `main`. Full detail in the 2026-06-18 session entry at bottom.
