...
> ## ▶ RESUME HERE — last updated 2026-07-18 (session 3)
> **RANGED CROSSHAIR + AIM-PARALLAX FIX COMPLETE, PIE-confirmed** ("all work fine" except one deferred cosmetic). A `WBP_RangedReticle` crosshair (`/Game/RPG/UI/`, simple UMG: canvas + 4 bars + dot) now shows in ranged stances (6 wand / 7 bow) and hides in melee, toggled in `ApplyStance` (lazy-create + SetVisibility, gated on `StanceIsRanged[CurrentStance]`). **Aim parallax FIXED in the same pass:** free-aim arrows now line-trace from the camera to the real hit point and aim the muzzle there (`LineTraceByChannel`→`Break`→`Select(hit?ImpactPoint:TraceEnd)`→the free-aim `FindLookAt.Target`), so the center crosshair is honest at close range. Lock-on aim path untouched.
> **▶▶ NEXT (open, user's call):** small polish backlog — (a) **DEFERRED lock-on crosshair** (user 2026-07-18 "leave as-is for now"): while locked, the center reticle sits at the enemy's feet (lock cam aims enemy−100Z by design) while the arrow homes to the body; later fix = hide reticle while locked (gate ranged AND !locked, updated in the lock/unlock handlers) or a dedicated on-target lock bracket — **don't touch the −100Z framing**; (b) still-deferred v2 ideas below (held-draw pose, charge-power, wand cast VFX, per-stance get-hit/death, the `BPC_PlayerStats` "Accessed None … As Dummy" bug — user: leave for now). **Tooling learned: UMG `WidgetTree` is NOT Python-exposed in 5.7 (crosshair built in designer by hand); `GetArrayItem` hits the wildcard-reconstruct trap like promotable operators. Full node GUIDs + detail in `.claude/handoff.md`.** **Uncommitted on `main` — user commits manually.**
>
> ## (session 2) RESUME — last updated 2026-07-16
